...
for root, dir, files in file_folder:
    for file in files:
        ...
        file_details = file_read(filepath) #finished reading content.

        if file_details in filecontents_store:
            #delete file
            os.remove(filepath)
            deleted_files +=1
            print(f"Duplicate file : {filepath} has been deleted")
            print(f"Original file  : {filecontents_store[file_details]}")

        else:
            filecontents_store[file_details] = filepath
